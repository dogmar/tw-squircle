...
Then pick the integration path that fits your project. Tailwind is the original target (paths A and B). Path C is the [Panda CSS](https://panda-css.com/) preset, and path D is the [StyleX](https://stylexjs.com/) helpers — both produce the exact same `@supports`-gated visual correction, just wired through their respective build pipelines.
...
For [StyleX](https://stylexjs.com/) projects, the package exposes parametric helpers and a factory — there's no baked-in radius set; you bring your own theme via `stylex.defineVars(...)`.

**Bundler setup:** add the package path to your StyleX bundler plugin's `include` config so the helpers are processed:

```ts
// vite.config.ts (or your StyleX plugin equivalent)
stylexPlugin({
  include: ["src/**/*", /node_modules\/@klinking\/squircle/],
});
```

**Factory pattern** — point at your own `defineVars` output and `stylex.create` the result:

```ts
import * as stylex from "@stylexjs/stylex";
import { defineSquircleVariants } from "@klinking/squircle/stylex";
...
const squircleStyles = stylex.create(defineSquircleVariants(radii));

<div {...stylex.props(squircleStyles.md)} />;
```

Pass `{ side: "topLeft" }` (or `"top"`, `"start"`, etc.) to target a per-side or per-corner property instead of `borderRadius`.

**Parametric helpers** — for ad-hoc use, spread the helper output into any `stylex.create(...)` block:

```ts
import { squircle, squircleTopLeft } from "@klinking/squircle/stylex";

const styles = stylex.create({
  card: { ...squircle("1rem"), padding: 16 },
  badge: squircleTopLeft("0.5rem"),
});
```

The full helper set mirrors the Panda table above (`squircle`, `squircleTop`, …, `squircleEndEnd`). Each one returns a StyleX style object whose `borderRadius` (or per-corner equivalent) and `cornerShape` properties carry per-property `@supports (corner-shape: superellipse(2))` conditional values, so unsupported browsers fall back to plain `border-radius`.

Override the superellipse exponent at call time by passing a second argument: `squircle("1rem", 3)` (number) or `squircle("1rem", "var(--my-amt, 2)")` (var expression). The CSS variable name read by default (`--squircle-amt`) is exported as `SQUIRCLE_AMOUNT_VAR`.
